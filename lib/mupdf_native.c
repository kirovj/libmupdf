...
JNIEXPORT jobject JNICALL
FUN(Document_openNativeWithBuffer)(JNIEnv *env, jclass cls, jbyteArray jbytes, jstring jmagic)
{
	fz_context *ctx = get_context(env);
	fz_document *doc = NULL;
	...
	if (jbytes)
	{
		...
		buffer = fz_new_buffer(ctx, len);
		bs = (*env)->GetByteArrayElements(env, jbytes, NULL);
		if (!bs) { jni_throw_io(env, "cannot get bytes to write"); return 0; }
	}

	fz_try(ctx)
	{
		fz_append_data(ctx, buffer, bs, len);
		stream = fz_open_buffer(ctx, buffer);
		doc = fz_open_document_with_stream(ctx, magic, stream);
	}
	fz_always(ctx) {
		if (bs)
			(*env)->ReleaseByteArrayElements(env, jbytes, bs, JNI_ABORT);
		if (magic)
